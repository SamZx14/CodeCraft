...
int ss_l1(int *life){
    ...
    c1(11);
    animation("\n\nType your answer here to complete the code and level up!",3);
    time_t start = time(NULL),time_diff; 
     char in_ans[10];
    while(1){
     ...
     c1(10);
      set_cnl_pos(0,24);
    if(strcmp(ans,in_ans)!=0){
        animation("For the explanation press Y, else press N\n",3);
    char ex_ch=_getch();
    if(ex_ch=='y'||ex_ch=='Y') {
        animation("\033[0;33mThe blank should be filled with \033[1;34mint\033[0;33m because \033[1;37mi\033[0;33m is an integer variable, and \033[1;34\033[0;33mint is used to declare integer types in \033[1;35mC.",4);
    }
    }
